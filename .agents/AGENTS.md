...
- `platform_kernel`'s `lib/src/utils/` (re-exported through `core_common`) is reserved for constants that are **genuinely global** — today only `ApiStatusConstants` (HTTP status codes) and `EnvConstants` (`String.fromEnvironment` values). Feature/domain-owned values (storage keys, route paths, API endpoints) MUST NOT live there.
