...
Package **names** did not change, deliberately. `core_di` lives at `platform/foundation/contracts` and is still
...
### Step 7 — Second app  *(7a, 7b done; 7c: the app exists and composes — building it needs a toolchain)*
...
| Stays in `apps/<id>/lib/` | Moves to `platform/shell/app_shell/` |
|:--|:--|
...
✅ **Prerequisites that need no toolchain — done:**

- **Firebase options belong to the app.** They name one bundle ID, yet lived in `core_common`,
  so every app would have booted as the mobile app's Firebase client. `FirebaseModule` is now
  `apps/mobile/lib/firebase/firebase_module.dart`; `core_notifications` moved from `core` to an
  `after` group, because its eager `PushNotificationService` injects those options and `before`
  runs ahead of the app's own registrations. `core_common` lost `AppConfig.firebase` and its
  `firebase_core` dependency.
- **Tools pick an app instead of assuming one.** `tools/shared/app_locator.dart` finds apps by
  `app_manifest.yaml` and takes `--app <id>`; `firebase_config.dart` (which still ran in the
  deleted `app/` directory) and `theme_setting.dart` use it, and `remove_sample.dart` now edits
  every app's manifest rather than only `apps/mobile/`.
- **`arch_check` recognises an app by its manifest**, not by the name `app` / `app_*` — a second
  app named `admin_app` would have been classified as core.

✅ **The app itself — `apps/admin` (`admin_app`).** `auth` + `settings`; no dashboard, splash,
onboarding, home, notifications or Firebase. Its `lib/` is a one-line `main.dart` and the
generated `injection.dart`, because the boot sequence moved into `runShellApp()` in
`platform_app_shell` first. Writing it surfaced three more defects that only a second app shows:
`composer sync --app <id>` rebuilt the root `workspace:` list from that app alone (dropping the
...
| 2026-09-24 | C4 | **The shell stops knowing the auth/home flow; module APIs leave the platform; group direction machine-checked (Option C, stage 4).** `core_di`'s shell-facing contracts became product-neutral session contracts in `src/session/` — `SessionPrincipal`, `SessionFailure` (+ 4 variants), `ISessionState`, `ISessionStatusStream` (`sessionStatusStream`), `ISessionRefreshListenable`, `ISessionGateway`, semantics unchanged — plus two location contracts next to `IAppEntryLocation`, `ISignInLocation` (auth: `AuthPath.LOGIN`) and `IPostSignInLocation` (home: `HomePath.HOME`), which `NavigatorWrapperWidget` resolves with `getItOrNull` and `context.go`es, falling back to `AppRouter.fallbackLocation`. `AuthNavigator` + `IAuthActionHandler` moved to the new `auth_api` (`modules/auth/api`), `HomeNavigator` to `home_api` (`modules/home/api`) — contracts only, `flutter` their one dependency; onboarding and settings depend on them. Composer: manifest layer `api`, no DI group — a workspace member, never an app dependency. `arch_check` **R11** (group DAG from `platform/<group>/<pkg>`, `dependencies:` only) and R3/R8/R1/R10 extended to API packages. `remove_sample` keeps an API package another package still imports (dry run + apply say so, manifest keeps `layers: [api]`); `docs_check` treats such a bundle as removed. New shell tests for the two locations. Lockfile unchanged. Verified in a throwaway clone: `remove_sample auth --apply` (keeps `auth_api`: onboarding, settings) and `remove_sample home --apply` (keeps `home_api`: onboarding) → composer sync → pub get → build_runner → analyze 0, arch_check, verify, docs_check (INFO only), every package test incl. both DI smoke tests. | ✅ pub get --enforce-lockfile, build_runner, barrels, analyze, 21 test packages (+4 shell tests for the location contracts) + tools (113), arch_check (31 packages), composer verify, dependency_sync, docs_check, unused-packages, `flutter build apk --flavor dev --debug` |
| 2026-09-24 | C2/C3 | **Group DAG obeyed; `platform_app_shell` split (Option C, stages 2–3).** Stage 2 removed every edge against the group direction instead of excusing it: `LoadMoreListView`/`LoadingMoreWidget` `core_ui_kit` → `provider_state_management` (ui no longer depends on state); `BottomTransitionPage` `core_common` → `core_ui_kit`, `AppInitializer`'s 600 px portrait threshold a private constant (foundation no longer depends on ui); the Dio → `AppFailure` mapping moved out of the kernel into `core_network`'s `DioFailureClassifier`, an eager DI singleton registering through the new `ErrorClassifier` / `ErrorHandler.registerClassifier` seam (kernel drops `dio`, 7 → 6 deps); `core_storage` → `platform_kernel` instead of `core_common` (it also used `DisposeGuard`: the 10-line guard is now inline in `StorageValue`); `data_auth` drops an unused `flutter`. Stage 3: `NetworkConfigImpl`, `NetworkBindingModule`, the theme/language storage adapters, `AppBootStorage` and their keys moved into the new `platform_shell_adapters` (`platform/shell/adapters`), first in each app's `shell` DI group; `platform_app_shell` depends on it and lost `core_network`/`core_storage`. Lockfile unchanged. Group graph checked by script: 0 violations. Enforcement in `arch_check` is the next stage. | ✅ pub get --enforce-lockfile, build_runner, barrels, analyze, 21 test packages (smoke tests now also require `AppBootStorage`/`NetworkConfig` and the Dio classifier; swapping adapters after `ui` fails them) + tools (92), arch_check, composer verify, dependency_sync, docs_check, unused-packages, `flutter build apk --flavor dev --debug` |
| 2026-09-24 | C1 | **`platform/` regrouped (Option C, stage 1).** The 15 platform packages moved by `git mv` into six role folders — `foundation/` (`kernel`, `contracts` = `core_di`, `common`), `layers/` (`domain`, `data`), `infra/` (`network`, `storage`, `database`, `notifications`), `ui/` (`responsive`, `design_system` = `core_base_ui`, `ui_kit`), `state/` (`provider`, `bloc`), `shell/` (`app_shell`). Package names unchanged; relative `path:` deps rewritten, composer regions regenerated, lockfile unchanged. `module_generator` types 4/5 gained `--group` (default `infra`). Group direction documented (`02_core.md` § 0) with three edges running against it today — enforcement is stage 2. | ✅ configure --stub-firebase, analyze, 21 test packages + tools (92), arch_check, composer verify, dependency_sync, docs_check, unused-packages, `flutter build apk --flavor dev --debug` |
| 2026-09-24 | loop+ | **Completion loop, rounds 4–5.** TLS pinning installed before the first widget (`initBeforeRunApp`); onboarding only on first launch; route observer attached; CI enforces the lockfile at the merge gate, builds a debug APK, and a dozen pipeline defects fixed (critical-count grep, script injection, altool key, Firebase CLI auth, symbols upload); tool input validation (`dependency_sync`, `composer`, generator name clashes, `configure --help`, 16 KB script, barrel/unused/docs_check edge cases) and English tool output; `tools/composer/bootstrap.dart` for partial checkouts; two literal newcomer walkthroughs (setup → every guide → sample removal) made every recipe compile as written; five READMEs split into en + vi. Final cross-document consistency audit: seven wording fixes. Commits `7e52ee8`…`ef3c2fa` and the one after | ✅ analyze, 16 test packages, all gates, `flutter build apk --flavor dev --debug` |
| 2026-09-24 | loop | **Completion loop, rounds 2–3.** Pref master key no longer replaced on a transient secure-storage error; Fastlane setup runs the barrel pass and uses iOS's `.staging` bundle id; nine runtime defects (repeated failure swallowed by the provider equality check, currency/phone formatters, load-more spinner, overlay insert assert, system back behind overlay dialogs, push-permission prompt blocking boot, case-sensitive blocked types, owned `FocusNode` leak, unscaled `core_ui_kit` defaults); `data_core` no longer declares `flutter`. New skill `implement_package_database`. Docs/skills/AGENTS re-audited against source in both locales. Commits `c49fcf4` `e71201c` `7f20a1e` `eb7af12` | ✅ analyze, 15 test packages (403 tests), arch_check, composer verify, dependency_sync, docs_check, unused-packages |
| 2026-09-23 | resp | **`core_responsive`: scale down by default, adaptive layouts.** Reported: an app-bar title far too big on a wide window. Cause, two layers: text scaling never ran (an M3 `ThemeData().textTheme` has no sizes, so the theme scaled `null`s) and the one style that did scale grew with width without limit (3.4x at 1280 vs the 375 artboard). Now: `ScaleBounds` for layout and text, both `downOnly()` by default, growth opt-in and capped; `ResponsiveProfile` per `WindowSizeClass` (Material 3 breakpoints, `ResponsiveBreakpoints`); adaptive widgets — `context.adaptive`, `AdaptiveLayout`, `AdaptiveSplitView` (hinge/fold aware: book and tabletop postures), `AdaptiveContent`. Samples: dashboard bottom bar → `NavigationRail` from medium, login form capped at 640. Built by two parallel subagents on a shared foundation, docs by a third, checked at 360/390/820/1280/700x400 on Linux by a fourth (no exception, overflow or clipping; it found the rail's off-brand indicator, fixed). 106 package tests; APK builds. Also: the foundation commit had tripped arch_check R4. Removal matrix: all seven sample bundles removable. Commits `1f19b44` … `4438559` | ✅ run |
| 2026-09-23 | verify+ | **Removability and the module generator, proven at runtime.** In a clean copy, `remove_sample auth --apply` → composer sync → build_runner → analyze clean, and the mobile app boots on Linux: onboarding → Home (signed out) → Settings (logout row hidden). That run found `HomeProfileBloc` taking `IAuthStatusStream` through its constructor — the one documented break — now an `@factoryParam` the route fills with `getItOrNull`; and the onboarding button stranding the user without auth — now falls back to `HomeNavigator`; `auth` has no `breaks` left. Generated a Provider/stack, a BLoC/tab and a no-SM feature: all gates clean, the generated tab boots and opens. Also fixed: composer derives workspace members from the pubspec graph (both apps drop `extra_dependencies`); the generator stops scaffolding unused deps; `check_unused_packages` reads `core_tools`; text scales by the window, not the physical display; read-pool connections get `busy_timeout` (S4 confirmed, regression test added); `dependency_sync` catalog comments; Gate 5 would have failed on a fresh CI checkout (local-only `build/` paths). Commits `2e1ee26` … `413d63b` | ✅ run |
| 2026-09-23 | verify | **First run on a real toolchain (Flutter 3.47.4, Android SDK 37).** It found two compile errors that broke every package on `core_common` (a lost import in `input_formatters.dart`, a missing `@visibleForTesting` import), a DI cycle that crashed the admin app at boot (`NetworkConfigImpl` resolved the auth gateway while `Dio` was being built), and CI Gates 0 and 2 that could never pass (the barrel pass reformatted composer's output and stripped `core_common`'s hand-written kernel re-export; composer emitted an `unnecessary_import`). All fixed. **Now verified here:** `flutter analyze` clean; `flutter test` 175/175 across 9 packages; `arch_check`, `composer verify`, `dependency_sync --check`, `docs_check` pass; `flutter build apk --flavor dev --debug` succeeds (with a stub `google-services.json`); `apps/admin` built for Linux boots to the login screen. Not possible here: an Android emulator (no KVM). Commits `f13da90` `a2a0b2a` `8eda8c3` | ✅ run |
| 2026-09-23 | runtime | **Runtime audit of platform/* and auth — 11 bugs, 5 latent defects fixed.** Worst: the retry dialog's buttons were wired backwards; a failed token refresh cleared the stored token but never the signed-in state (user stayed on screen with no token); any refresh failure — even a timeout — signed the user out; the retry queue lost requests sharing a path and could double-complete or double-send; a first-launch race could wipe the preferences master key; `fallbackLocation` sent a signed-in user to onboarding, and with no destinations the app opened on the not-found page. Also: typed-list and custom-type storage, nullable `execute`, `setData(null)`, WAL loss on quarantine, migrations not scoped per database, credentials in logged bodies, a leaked network listener. Commits `a24a7a9` `3255fb2` `b3e0b52` `ad55f7f` `e941998` `1c9d3ef` | ⚠️ tests added, not run here |
| 2026-09-23 | tools | **Tools audit — 20 confirmed bugs fixed.** An independent pass read every tool against its docs. Worst: `dependency_sync --check` (CI Gate 4) failed on the tree because inline `# comments` read as version drift, and ran `pub get` in check mode; `code_review` refused to run from the repo root (every documented call and both CI steps) and printed via ~180 `print()`s; the module generator put its workspace entry outside the composer markers (a permanent duplicate), deleted an existing target before the rollback snapshot, and generated a `.gitignore` that would commit generated code; `firebase_config` configured staging's Android client for `.staging` while Gradle builds `.stg`; `composer sync --strict` wrote before failing; `check_outdated` applied every update on a bare Enter; `check_unused_assets` could never report an unused asset. FVM detection is now one shared helper (`tools/shared/toolchain.dart`). Commits `d7b125b` `0a260ee` `dbf5bc2` `0c815f0` `b1ce84d` `455f71a` | ⚠️ not run — no SDK here |
| 2026-09-23 | 9 | **Third doc audit; the snippet check had a blind spot.** The local snippet comparison only matched `Real code from [link]` on one line, so blocks introduced across two lines were never compared — `10_cross_feature` still showed `UserEntity` in the `IAuthStatusStream` contract, `02_new_domain_data` a `UserModel` with bank fields and a `getAll()` the repository no longer has. With that fixed, plus an independent agent pass, ~45 claims corrected in both locales (R8-blocked `getIt` examples, wrong annotations, stale names, lists missing the cache module, AGENTS § references). Commits `cf82bb1` `d82f375` `09f701d` | ⚠️ not run — mechanical checks clean |
| 2026-09-23 | 6a+ | **Two tool regressions.** `configure.dart` lost its `build_runner` step in `e0cf853`; with no generated file committed, setup — and CI's codegen step, which calls it — produced nothing. Restored. The module generator built its path from a display string and wrote to a literal `modules/*/feature/<name>`; layers now land at `modules/<name>/<layer>`. Commit `f5c88d7` | ⚠️ not run |
| 2026-09-23 | net | **Refresh could deadlock.** Login and refresh went through the refresh interceptor like any call: a wrong password triggered a token refresh, and a `401` from the refresh call awaited the `Completer` that was awaiting it. `core_network` gains `EXTRA_CAN_REFRESH_TOKEN` (with a test); `AuthRemoteDataSource` sets it on both. Commit `fb2c9cb` | ⚠️ not run — new unit test added, not executed |
| 2026-09-23 | 2i | **Platform residue from earlier products removed.** Unused colour slots (`chatMe`, three `liquid*` palettes); `DownloadImage`, the Dio extension, `SocketState`, `UpdateStatus`, `ApiStatusConstants`, `AppUtils.requestPermission` and file-size helpers; sample notification channel ids; env keys no code read (Maps, Facebook, socket, schema — env files now carry 4 keys). Drops `dio` and `permission_handler` from `core_common`, `freezed`/`json_annotation` from `core_di`. Declared `flutter_localizations`/`intl` in every package that runs `gen-l10n`. Commits `a7a927a` `cce9b08` `4c6f3e9` `386df69` `b5cfda5` `4e64434` | ⚠️ not run |
| 2026-09-23 | 2h | **The Drift example is a module now.** The cache chain moved out of the framework packages `domain_core` / `data_core` into `modules/cache/{domain,data}` (`domain_cache`, `data_cache`), trimmed to what it demonstrates (get/save; `GetAllCacheEntriesUseCase`, `delete`, `getAll`, `clearAll` had no caller). `data_core` no longer depends on `core_database` or `drift`. `apps/mobile` composes `cache`; `apps/admin` composes neither it nor `core_database`, so it opens no SQLite file at boot. The database tests moved with their fixture. `sample_manifest.yaml` lost its `embedded_samples` section: no framework package contains sample code any more. | ⚠️ not run |
| 2026-09-23 | 2g | **A build break, and a sample trim.** `feature_settings` referenced `SettingsPath.SETTINGS` and `$settingsShellRoute`, neither of which exists — neither app compiled; fixed, and a scratch check now resolves every `XxxPath.CONST` / `$xxxRoute` in the repo. `sample_manifest.yaml` corrected (the auth REST source is live; cache_chain's real removal set). Trimmed ~200 lines without changing behaviour: cache_chain banners, dashboard's empty localization scaffold, the unused `SettingsNavigator` / `OnboardingNavigator`, `LoginParams.fromJson`, history comments. **Deferred, needs a decision and a toolchain:** cache_chain's domain half (entity, params, repository, 3 use cases) repeats what auth already shows, and `data_core` opens its SQLite file on every boot for code nothing uses. Commit `04d0a0d`. | ⚠️ not run |
| 2026-09-23 | 9 | **Second semantic doc audit, all Markdown.** An independent pass compared every `.md` except this log against the code and reported ~45 false statements; each was verified before fixing. Main clusters: `platform_kernel`'s contents still attributed to `core_common`; `domain_core`/`data_core` placed under `modules/`; the deleted `language` module still listed; stale counts (workspace members, approved edges, CI gates, test packages); the `notifications` DI group missing from order descriptions; the app-shell doc and `08_networking.md` showing pre-3c code; `.github` guides describing workflows that do not exist. Commit `1b816b0`. | ⚠️ not run |
| 2026-09-23 | 7c | **`apps/admin` exists.** Second app composing `auth` + `settings` only; `lib/` is `main.dart` + generated `injection.dart`. Prerequisites landed with it: `runShellApp()` in `platform_app_shell` (the boot sequence was in `main.dart`, so a second app would have copied it); `composer sync --app` now keeps the root `workspace:` list a union over **all** apps (it was built from the selected app only); the router falls back to the bare `navigationShell` without a dashboard (was a blank `SizedBox`); `apps/mobile/pubspec.yaml` lost 18 dependencies nothing in the app uses any more (13 regular, 5 dev). `docs_check` now scans every Markdown file in the repo — the old four roots missed `README.vi.md`, `tools/README.md`, the `.github` guides and package READMEs, where 11 dead links had collected. `code_review.yml` watches `apps/*`. | ⚠️ not run |
| 2026-09-23 | 7c− | **Firebase is per app; tools stop assuming one app.** `FirebaseModule` moved from `core_common` to `apps/mobile/lib/firebase/` (options name one bundle ID); `core_notifications` moved to a new `notifications` group in `after`, since its eager `PushNotificationService` injects the app-registered `FirebaseOptions`; CI's stub step now loops over `apps/*/lib/firebase/`. New `tools/shared/app_locator.dart` (`--app <id>`) used by `firebase_config.dart` — which had still run in the deleted `app/` directory with an `--out` pointing into core — `theme_setting.dart` and `remove_sample.dart`. `arch_check` detects apps by `app_manifest.yaml`. `module_generator`: rollback now snapshots the manifests it actually edits (it snapshotted two files it no longer writes), and a dead helper that edited `root_app.dart` is gone. `unused_checker` treats injectable-annotated files as entry points. Docs: README DI snippet replaced with the generated form (it still listed deleted `DomainLanguage`/`DataLanguage` modules and no shell); `02_core.md` dependency map regenerated from pubspecs (4 rows were stale); DI skill step 3 told agents to hand-edit the generated `injection.dart`. | ⚠️ not run |
| 2026-09-23 | 7c− | **Two boot bugs, found by reading the boot path as an app without onboarding and home would run it.** (1) `_goToOnboarding()` returned `true` on every first launch whether or not an onboarding module existed, so any build without one started on its fallback tab, returned early, and skipped the login redirect — first launch opened signed-out on a protected screen. (2) After sign-in, `getItOrNull<HomeNavigator>()?.toHome()` was a no-op without a home module, leaving a signed-in user on the login screen. Both invisible so far because every sample app composed onboarding and home. `AppRouter.fallbackLocation` is now public and the single definition — `UndefineRouteWidget` had its own copy. Also: `docs_check` now validates placeholder and glob paths (≥1 real match), which caught 42 paths the relayout had mangled into `modules/*/<layer>/<x>` order (fixed to `modules/<x>/<layer>`); its limit — a placeholder in the last segment matches anything — is written into the tool. | ⚠️ not run |
| 2026-09-23 | 7b | **The app shell is a package.** 20 files moved from `apps/mobile/lib/` to `platform/shell/app_shell/` (`platform_app_shell`); what stays is `main.dart` and the generated `injection.dart`. New `shell` DI group, first in `after` — the slot app-local registrations always occupied, so boot order is unchanged. The package imports no module, and now R1 holds that instead of R10. Regenerated artifacts with a composer port validated byte-for-byte against the committed output. Docs rewritten rather than patched: `06_app_shell.md` (both locales) now describes the two-part shell; `05_di.md` §3 had described a `_databaseModules` group that does not exist and a `CoreDatabasePackageModule` that "runs LAST" while `injection.dart` runs it first; and 19 places still said `NetworkConfigImpl` injects `AuthLocalDataSource`, untrue since step 3c. | ⚠️ not run |
