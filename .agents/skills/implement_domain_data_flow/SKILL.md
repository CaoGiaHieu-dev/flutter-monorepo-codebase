...
- **DataSources return Models**, never Entities — and never a type generated by Drift. The
  one allowed wrapper is `domain_core`'s `BaseEntity<T>` response envelope (the reference
  `AuthRemoteDataSource` returns `Future<BaseEntity<UserModel>>`). Unwrapping the envelope and
  mapping to Entities happens in the RepositoryImpl.
- Constants for a package live in its own `lib/src/utils/` — API paths included
  (`modules/auth/data/lib/src/utils/auth_api_constants.dart`), never in `core_common`.
...
Then declare the new packages explicitly in the consuming `pubspec.yaml` files, under
`dependencies:` — Pub Workspaces will happily compile an undeclared import. Verify with
