...
│       ├── code_review.yml        # Gemini AI review on pull requests
...
│   ├── docs_check/                # Every repo path the docs name exists — PR Gate 5
│   ├── firebase/                  # Per-flavor Firebase configuration for one app
│   ├── module_generator/          # Scaffolds Feature/Domain/Data/Core/Custom packages
│   ├── sample_cleanup/            # Lists and removes the sample modules safely
│   ├── shared/                    # Helpers the tools share (FVM detection, app locator)
...
- **Dart SDK**: >= 3.13.3
- **JDK**: 17 or newer (17 is the bytecode target; the build runs on 21)
...
```
This is **the** setup step: `flutter clean` → `flutter pub get` → `gen-l10n` in every package with an
`l10n.yaml` → `dart run build_runner build --workspace` → the barrel generator for every package
(it also activates `flutterfire_cli` first). `pub get` + `build_runner` alone is **not** equivalent.
The gitignored `lib/src/gen/gen.dart` barrels only exist after the barrel pass, and without them
`flutter analyze` fails with about 17 errors (`gen/gen.dart` missing, `AppLocalizations` / `Assets`
undefined). The full manual sequence is in
[`getting-started/01_setup.md`](docs/en/getting-started/01_setup.md) § 2.
...
- **Have none yet:** create the compile-only stubs, the three Dart files plus a `google-services.json`
  per flavor. The app builds, but push and other Firebase features do not work.
