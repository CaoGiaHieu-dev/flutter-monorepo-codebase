# `apps/admin` — the second app

A back-office app built from the **same modules** as `apps/mobile`, composing a different
subset of them: `auth` and `settings`, and none of `dashboard`, `splash`, `onboarding` or
`home`. It exists to keep the composition design honest. Every optional lookup in
`platform_app_shell` (`getItOrNull`, `getAllOrEmpty`) has a missing contribution in this app,
permanently, instead of only in a thought experiment:

| Not composed | What the shell does instead |
|:--|:--|
| `feature_splash` (`IAppSplashScreen`) | keeps the native splash through boot |
| `feature_onboarding` (`IAppEntryLocation`) | starts on the first destination and goes straight to the login check |
| `feature_home` (`IPostSignInLocation`) | after sign-in, goes to `AppRouter.fallbackLocation` — the settings tab |
| `feature_dashboard` (`IDashboardRouteModule`) | renders the destinations without chrome |
| `core_notifications` + Firebase | nothing: no `notifications` DI group, no `lib/firebase/` |
| `cache` (the Drift sample) | no SQLite file is opened at boot |

## What is in here

```
apps/admin/
├── app_manifest.yaml      which modules, which DI group order — the only file you edit to change the app
├── pubspec.yaml           path dependencies generated between composer markers
├── env.dev                --dart-define values (no secrets; env.prod is yours to create)
├── lib/
│   ├── main.dart          runShellApp(configureDependencies: configureDependencies)
│   └── di/
│       └── injection.dart generated by composer (injection.config.dart by build_runner)
└── test/
    └── di_smoke_test.dart boots this app's real DI graph for every flavor (RULE-63)
```

That is the whole app. The boot sequence, router and material wrapper come from
[`platform/shell/app_shell`](../../platform/shell/app_shell/), the storage adapters and
`NetworkConfigImpl` from [`platform/shell/adapters`](../../platform/shell/adapters/).

To change what it is made of, edit `app_manifest.yaml`, then from the repository root:

```bash
dart tools/composer/composer.dart sync --app admin
flutter pub get
dart run build_runner build --workspace
```

On a fresh clone, run `dart tools/workspace_setup/configure.dart` once first. It is the workspace
setup step, and `pub get` + `build_runner` alone leave the generated barrels missing. See
[`docs/en/getting-started/01_setup.md`](../../docs/en/getting-started/01_setup.md) § 2.

## Running it

No platform runner folders are committed yet — the app is meant for desktop, and which desktop
platforms a team ships is its own decision. Generate them once, from this directory:

```bash
cd apps/admin
flutter create --platforms=macos,windows,linux --org com.example --project-name admin_app .
```

`flutter create` leaves existing files alone and adds what is missing. It also writes two files
to delete:

- `test/widget_test.dart`, a test for a counter app that does not exist here.
- `analysis_options.yaml`, whose `flutter_lints` include replaces the repository-root analysis
  options for everything under `apps/admin/`.

```bash
rm test/widget_test.dart analysis_options.yaml
```

Then, still from `apps/admin/`:

```bash
flutter run -d macos --dart-define-from-file=env.dev
```

Notes that apply to this app specifically:

- **Desktop first; the web works too.** This app has been built for the web and loaded in a
  browser: after `flutter create --platforms=web .` it builds with `flutter build web` and boots
  to the sign-in screen, with secure storage and preferences working (the page must be a secure
  context: `https` or `localhost`). On the web the browser owns TLS, so `AppInitializer` installs
  no `HttpOverrides` and there is no SSL pinning. The measurement and what `apps/mobile` still
  lacks on the web are in [`docs/en/architecture/02_core.md`](../../docs/en/architecture/02_core.md)
  § 11. This app opens no database: it composes neither the `cache` module nor `core_database`,
  so no Drift or SQLite code is in its dependency graph.
- **Flavor.** `AppConfig.appFlavor` reads the flavor the app was built with. With none, it falls
  back by build mode: `dev` in a **debug** build, so a plain `flutter run` registers the `dev` DI
  environment, and `prod` in a profile or release build. TLS is stricter still: certificate
  validation is bypassed only in a debug build that *declared* `--flavor dev`, so a build with no
  flavor keeps validation on (and logs an ERROR) even when its DI environment is `dev`.
- **Branding.** `tools/theme_generator/theme_setting.dart --app admin` is refused. The tool checks
  every precondition before writing anything, and requires the app to have both `android/` and
  `ios/` — the shared `flutter_native_splash` / `icons_launcher` configs at the repository root
  enable both — as well as both packages in its `dev_dependencies`. This app has neither platform
  directory, so adding the `dev_dependencies` alone does not unblock it; it stays refused until
  the app gains `android/` and `ios/`. This app does not use Firebase.

## What checks it today

`composer verify` (CI Gate 0) regenerates this app's composition and fails on drift, and
`arch_check` R10 fails if anything under `lib/` other than `injection.dart` imports a module.
CI Gate 3 runs `test/di_smoke_test.dart`, which boots this app's real DI graph for every flavor —
so a missing registration or a DI ordering mistake in this composition fails the PR. What CI does
not do is build a binary of it: the build job builds `apps/mobile` only.
