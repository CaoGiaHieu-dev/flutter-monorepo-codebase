...
This is a **Pub Workspace**. There is exactly one dependency resolution for all 26 workspace members (23 packages, the two apps, and `tools`), so you run `pub get` **once, at the repo root** — never inside a sub-package.
