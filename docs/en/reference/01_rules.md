...
**Why the Model rule.** Returning a Drift row class leaks the persistence library into every consumer of the package. `CacheEntryModel` (`platform/data_core/lib/src/models/cache_entry_model.dart`) exists purely as that boundary.
