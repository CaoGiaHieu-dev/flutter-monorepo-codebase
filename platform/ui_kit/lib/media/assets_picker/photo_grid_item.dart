...
class _PhotoGridItemState extends State<PhotoGridItem>
  ...
  Future<void> _loadThumbnail() async {
    if (widget.thumbnailCache.containsKey(widget.photo)) {
      ...
      // Resolve the decode size from context *before* the first await —
      // reading it afterwards would touch a possibly-unmounted element.
      // Scheduled at endOfFrame from initState, so the element is mounted
      // and the inherited scope is available here.
      if (!mounted) return;
      final thumbnailSide = context.w(200).toInt();
